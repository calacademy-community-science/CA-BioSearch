...
server <- function(input, output) {
    ...
    full_query <- eventReactive(input$submitquery, {
        ...
        paste(
            ...
            "Limit to 5000 records. Return only the SQL query with no explanation or other text.",
            collapse = " "
        )
    })
    ...
    observeEvent(observations.df(), {
        ...
        if (nrow(poly_obs.sf) > 0) {
            print("polygon!")
            poly_obs.sf <- poly_obs.sf %>%
                # filter(if_any(contains("geom"), ~ !is_double(.))) %>% 
                select(names(.)[colSums(is.na(.)) != nrow(.)]) %>% 
                # rename_with(~ gsub("geom*", "geom", .), contains("geom")) %>%
                rename_with(~ ifelse(grepl("geom", .), "geom", .), contains("geom")) %>% 
                st_as_sf(sf_column_name = "geom", na.fail = F)
        } else {
